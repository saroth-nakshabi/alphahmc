...
@section('content')
    <div class="card bg-light-info shadow-none position-relative overflow-hidden">
        <div class="card-body px-4 py-3">
            <div class="row align-items-center">
                <div class="col-9">
                    <h4 class="fw-semibold mb-8">All Google tags</h4>
                        ...
                        <div class="table-responsive">
                            <table id="items-table" class="table border table-striped table-bordered display text-nowrap">
                                <thead>
                                    <!-- start row -->
                                    <tr>
                                        <th>Google Tag Name</th>
                                        <th>Action</th>
                                    </tr>
                                    <!-- end row -->
                                </thead>
                                <tbody>
                                    @if (isset($googletags) && count($googletags) > 0)
                                        @foreach ($googletags as $google)
                                            <!-- start row -->
                                            <tr data-id="{{ $google->id }}">
                                                <td>{{ $google->googletag_name }}</td>
                                                        ...
                                                        <ul class="dropdown-menu">
                                                            @can('edit tags')
                                                                <li><a class="dropdown-item edit" href="javascript:void(0);" data-bs-toggle="modal" data-bs-target="#editModal"
                                                                        data-id="{{ $google->id }}">Edit</a></li>
                                                            @endcan
                                                            @can('delete tags')
                                                                <li><a class="dropdown-item delete" href="javascript:void(0);"
                                                                        data-id="{{ $google->id }}">Delete</a></li>
                                                            @endcan
                                                        </ul>
                                                    </div>
                                                </td>
    ...
    <div class="modal fade" id="addNewModal" tabindex="-1" aria-labelledby="addNewModal" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
            <form class="modal-content" action="{{ route('google.store') }}" method="POST" id="add_form"
                ...
                <div class="modal-body">
                    <div class="row pt-3">
                        ...
                        <div class="col-12">
                            <div class="mb-3">
                                <label class="control-label mb-1">google tag Name <span class="text-danger">*</span></label>
                                <input type="text" id="name" name="name" class="form-control"
                                    placeholder="google tag Name" />
                            </div>
                        </div>
    ...
    <div class="mb-3">
        ...
        <textarea id="tags" name="tags" rows="10" class="form-control"
            placeholder="Paste your script tags here..." required></textarea>
    </div>
</div>
                        <!-- In both add and edit modals, add this field -->
<div class="col-md-12">
    <div class="mb-3">
        <label class="control-label mb-1">
            NoScript Tags 
            <small class="text-muted">(paste the &lt;noscript&gt; snippet here)</small>
        </label>
        <textarea id="noscript_tags" name="noscript_tags" rows="5" 
            class="form-control" placeholder="<noscript>...</noscript>"></textarea>
    </div>
</div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <form class="modal-content" method="POST" id="edit_form" enctype="multipart/form-data">
                ...
                <div class="modal-header">
                    <h4 class="modal-title">Edit Google Tags</h4>
    ...
    <textarea id="edit_tags" name="tags" class="form-control" rows="10" required></textarea>
                        ...
                        <div class="col-md-12">
                            <div class="mb-3">
                                ...
                                <textarea id="edit_noscript_tags" name="noscript_tags" rows="5" 
                                    class="form-control" placeholder="<noscript>...</noscript>"></textarea> {{-- value="{{ old('noscript_tags', $googletag->noscript_tags) }}"--}}
                            </div>
                        </div>
...
@section('custom_js')
   ...
   <script>
        $(document).ready(function() {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            const newRow = `<tr data-id="${response.data.id}">
                                <td>${response.data.googletag_name}</td>
                                <td>
                                    <div class="btn-group">
                            ...
                            $('#add_form')[0].reset();
                            addEditor.setValue(''); // clear CodeMirror after reset
                        },
                        error: function(xhr) {
                            Swal.close();
                            let errorMessages = '';
            ...
            // -----------------------------------------------
            // Edit button - fetch and populate
            // -----------------------------------------------
            $(document).on('click', '.edit', function() {
                const id = $(this).data('id');
                const updateUrl = `{{ route('google.update', '') }}/${id}`;

                $('#edit_form').attr('action', updateUrl);

                $.ajax({
                    url: `{{ route('google.get') }}`,
                    method: 'POST',
                    data: { id: id },
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    ...
                    success: function(response) {
                        $('#edit_name').val(response.data.googletag_name);
                        $('#edit_noscript_tags').val(response.data.noscript_tags ?? '');

                        // Set content in CodeMirror (not TinyMCE)
            ...
            $(document).on('click', '.delete', function() {
                const id = $(this).data('id');
                const deleteUrl = `{{ route('google.destroy', '') }}/${id}`;
