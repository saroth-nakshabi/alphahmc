...
@section('content')
    <div class="card bg-light-info shadow-none position-relative overflow-hidden">
        <div class="card-body px-4 py-3">
            <div class="row align-items-center">
                <div class="col-9">
                    <h4 class="fw-semibold mb-8">All Project Categories</h4>
    ...
    <div class="modal fade" id="addNewModal" tabindex="-1" aria-labelledby="addNewModal" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg">
            <form class="modal-content" action="{{ route('project_categories.store') }}" method="POST" id="add_form">
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.edit', function() {
                ...
                const udpateUrl = `{{ route('project.category.update', '') }}/${id}`;
                $('#edit_form').attr('action', udpateUrl);

                $.ajax({
                    url: `{{ route('project.category.get') }}`,
                    method: 'POST',
                    data: {
                        'id': id,
                    },
            ...
            $(document).on('click', '.delete', function() {
                const id = $(this).data('id');
                const deleteUrl = `{{ route('project.category.destroy', '') }}/${id}`;
